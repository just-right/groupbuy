...
import './MerchantCheck.css';
import moment from 'moment';
const confirm = Modal.confirm;
class MerchantCheck extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
    ...
    initfunction() {
        fetch('/userservice/api/merchant/list/apply', {
            method: 'GET',
            headers: {
    ...
    showConfirm(item, index) {
        ...
        confirm({
            ...
            onOk: () => {
                ...
                fetch('/userservice/api/administrator/' + merchant_id + '/merchantcheck/', {
                    method: 'GET',
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json',
    ...
    showDeleteConfirm(item,index) {
        confirm({
            ...
            onOk: () => {
                ...
                const merchant_id = item.merchant_ID;
                fetch('/userservice/api/administrator/' + merchant_id + '/merchantdelete/', {
                    method: 'DELETE',
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json',
        ...
        return (
            <div>
                ...
                >
                    ...
                    <p><span className='tip'>申请注册时间：</span><span className='value'>{moment(this.state.tmpdata.registrationDate).format('YYYY-MM-DD HH:mm:ss')}</span></p>
