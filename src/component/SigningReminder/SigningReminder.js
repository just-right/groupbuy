...
import './SigningReminder.css';
import cookie from 'react-cookies';
const confirm = Modal.confirm;
class SigningReminder extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
            ...
            tmpdataList: [],
            page: 8,
            status: ['订单已在购物车', '订单已支付', '订单已发货', '客户已收货', '客户已评价'],
            count: 0,
            visible: false,
            emptyFlag: false,
            userName: '',
    ...
    showSignInfo(index) {
        ...
        const userid = this.state.tmpdataList[index].user_id;
        const orderid = this.state.tmpdataList[index].order_id;
        const product_name = this.state.tmpdataList[index].product_name;
        const addUrl = '?userid='+userid+'&orderid=' + orderid + '&productname=' + product_name;
        fetch('/orderservice/api/order/message/create/' + addUrl, {
            method: 'POST',
            headers: {
    ...
    initOrder() {
        ...
        fetch('/orderservice/api/order/shiplist/by/merchant/' + merchant_id, {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
                    ...
                    renderItem={(item, index) => (
                        <List.Item>
                            <List.Item.Meta
                                ...
                                description={<span><Button onClick={this.showModal.bind(this, item)} type="primary" ghost>详&nbsp;&nbsp;情</Button>&nbsp;&nbsp;<Button onClick={this.showSignInfo.bind(this, index)} type="primary" ghost>提醒用户签收</Button></span>}
                            />
                        </List.Item>
                    )
                    }
...
export default SigningReminder;